...
func (s *MemorySeriesStorage) maybeEvict() {
	...
	if s.targetHeapSize > heapSize*10 {
		// Heap is still tiny. Don't do anything. Leave urgency at 0.
		return
	}
	if numChunksToPersist > 0 {
		newUrgency = int32(numChunksToPersist * 1000 / (numChunksToPersist + s.evictList.Len()))
		if newUrgency >= 1000 {
			newUrgency = 999 // Never throttle because of the chunk ratio.
		}
	}
...
// time. Capped to 1.
//
// If the ingestion pressure is 1, the Prometheus process has a heap size larger
// than it should have, but no more chunks can be evicted. Ingestion will be
// throttled to mitigate.
//
// If the heap size is less than 10% of the configured max size, Prometheus is
// considered to be in a warm-up phase and the score is fixed to 0.
//
